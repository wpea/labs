...
export default function Stocks() {
  ...
  const getStocks = async () => {
    const res = await fetch(
      "https://yh-finance.p.rapidapi.com/market/v2/get-quotes?region=US&symbols=TSLA,JNJ,NVDA,HD,FB,MSFT,CSCO,GOOG,ADBE,PG,TMO,LLY",
      {
        method: "GET",
        headers: {
  ...
  const Th = ({ data }) => {
    return data.map((t) => (
      <div key={t} className="grid items-center border-r py-4 text-xs font-medium tracking-tight">
        {t}
      </div>
    ));
  };
              ...
              {purchased.map((p) => (
                ...
                >
                  <div className="grid items-center border-r py-4 text-xs tracking-tight">
                    <div>
                      <div className="font-bold text-blue-700">{p.symbol}</div>
                  ...
                  />
                  <Td
                    value={currency(commission(p.shares, p.buy_price)).format()}
                  />
                  <Td
                    value={currency(commission(p.shares, p.buy_price)).format()}
                  />
                  <Td
                    value={currency(initValue(p.shares, p.buy_price)).format()}
                  />
                  ...
                  <Td
                    value={currency(pfvCurrent(p.symbol, p.shares)).format()}
                  />
                  ...
                  >
                    ...
                    <div className="flex justify-between">
                      ...
                      <div className="font-semibold">
                        {currency(ptvp(p.symbol, p.shares, p.buy_price)).value}
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between">
                      <div>$</div>
                      <div className="font-semibold">
                        {currency(ptr(p.symbol, p.shares, p.buy_price)).value}
                      </div>
                    </div>
                  </div>
